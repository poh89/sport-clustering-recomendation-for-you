...
# Logic ให้เหตุผลแบบเดิม + อ้างอิงพื้นฐาน
def get_original_expert_reason(row, old_row, user_req, is_newbie):
    reasons = []
    
    # ดึงคำอธิบายตามค่า Slider (แบบเดิม)
    if user_req[0] >= 7: reasons.append(f"เน้นการฝึกหัวใจ (Cardio) ระดับ {user_req[0]} ตามเป้าหมายของคุณ")
    if user_req[2] <= 4: reasons.append(f"ใช้งบประมาณน้อย (ระดับ {row['Budget']}) เริ่มได้ทันที")
    
    # อ้างอิงกรณีเคยเล่นกีฬา (แบบ Expert)
    if not is_newbie and old_row is not None:
        if row['Intensity'] > old_row['Intensity']:
            reasons.append(f"ท้าทายความอึดได้มากกว่า {old_row['Sport']} ที่เล่นอยู่")
        reasons.append(f"ช่วยเสริมสร้างทักษะใหม่ที่ส่งเสริมพื้นฐานจาก {old_row['Sport']}")
    elif is_newbie:
...
# --- [3. UI INTERFACE] ---
# ส่วนตั้งค่าแหล่งข้อมูล (Sidebar เดิม)
with st.sidebar:
    ...
    st.divider()
    st.info("💡 ระบบจะดึงข้อมูลจากแหล่งที่เลือกมาคำนวณความเหมาะสม")

st.title("🏆 SportMatch AI Expert")
st.caption("ระบบวิเคราะห์กีฬาอัจฉริยะด้วย Machine Learning")

# แถวข้อมูลผู้ใช้งาน (เหมือนรูป image_04cd89.png)
...
with col_left:
    ...
    if experience == "เคยเล่นกีฬาบางชนิดมาก่อน":
        ...
        st.success(f"ดึงทักษะจาก {selected_old}")
    ...
    for i, (idx, row) in enumerate(recs.iterrows(), 1):
        with st.container():
            ...
            with c_info:
                ...
                # ใช้ฟังก์ชันเหตุผลเดิมที่รวม Expert เข้าไปด้วย
                reason = get_original_expert_reason(row, old_sport_row, user_req, is_newbie)
